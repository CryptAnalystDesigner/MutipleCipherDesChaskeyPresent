...
def first_stage(n, num_rounds=7, pairs=8):
    ...
    with mp.Pool(process_number) as pool:
        accept_XY = pool.starmap(des.make_dataset_with_group_size, [(int(
            n*pairs/process_number), num_rounds-3, (0x04000000, 0x40080000), pairs,) for i in range(process_number)])
    ...
    with mp.Pool(process_number) as pool:
        accept_XY_eval = pool.starmap(des.make_dataset_with_group_size, [(int(
            test_n*pairs/process_number), num_rounds-3, (0x04000000, 0x40080000), pairs,) for i in range(process_number)])
...
def second_stage(n, num_rounds=9, pairs=8):
    ...
    with mp.Pool(process_number) as pool:
        accept_XY = pool.starmap(des.make_dataset_with_group_size, [(int(
            n*pairs/process_number), num_rounds, (0x40080000, 0x04000000), pairs,) for i in range(process_number)])
    X = accept_XY[0][0]
    Y = accept_XY[0][1]
    for i in range(process_number-1):
        X = np.concatenate((X, accept_XY[i+1][0]))
    ...
    with mp.Pool(process_number) as pool:
        accept_XY_eval = pool.starmap(des.make_dataset_with_group_size, [(int(
            test_n*pairs/process_number), num_rounds, (0x40080000, 0x04000000), pairs,) for i in range(process_number)])
    X_eval = accept_XY_eval[0][0]
    Y_eval = accept_XY_eval[0][1]
