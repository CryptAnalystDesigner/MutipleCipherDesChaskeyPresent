...
def first_stage(n, num_rounds=7, pairs=8):
    ...
    with strategy.scope():
        net = load_model("../DATA_N_good_trained_nets/present_best_"+str(num_rounds-1) + 'r_pairs' +
        ...
        net_first.load_weights("../DATA_N_good_trained_nets/present_best_"+str(num_rounds-1) + 'r_pairs' +
                               str(pairs) + '_distinguisher.h5')

    process_number = 50
    with mp.Pool(process_number) as pool:
        accept_XY = pool.starmap(present.make_dataset_with_group_size, [(int(
            n/process_number), num_rounds-2, 0x0000000002000100, pairs,) for i in range(process_number)])
    ...
    with mp.Pool(process_number) as pool:
        accept_XY_eval = pool.starmap(present.make_dataset_with_group_size, [(int(
            test_n/process_number), num_rounds-2, 0x0000000002000100, pairs,) for i in range(process_number)])
...
def second_stage(n, num_rounds=9, pairs=8):
    ...
    with mp.Pool(process_number) as pool:
        accept_XY = pool.starmap(present.make_dataset_with_group_size, [(int(
            n/process_number), num_rounds, 0x9, pairs,) for i in range(process_number)])
    X = accept_XY[0][0]
    Y = accept_XY[0][1]
    for i in range(process_number-1):
        X = np.concatenate((X, accept_XY[i+1][0]))
    ...
    with mp.Pool(process_number) as pool:
        accept_XY_eval = pool.starmap(present.make_dataset_with_group_size, [(int(
            test_n/process_number), num_rounds, 0x9, pairs,) for i in range(process_number)])
    X_eval = accept_XY_eval[0][0]
    Y_eval = accept_XY_eval[0][1]
