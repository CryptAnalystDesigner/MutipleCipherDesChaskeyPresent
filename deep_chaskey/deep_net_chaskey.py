...
def train_chaskey_distinguisher(num_epochs, num_rounds=7, diff=(0x8400, 0x0400, 0, 0), group_size=2, depth=1):
    ...
    with mp.Pool(process_number) as pool:
        # accept_XY = pool.starmap(chk.create_train_test_dataset, [(int(10**7*group_size/process_number), int(
        #     10**6*group_size/process_number), num_rounds, 0, 0, group_size, diff,) for i in range(process_number)])
    ...
    check = make_checkpoint(
        wdir+'chaskey_best_'+str(num_rounds)+'r_pairs'+str(group_size)+'_distinguisher.h5')
    lr = LearningRateScheduler(cyclic_lr(10, 0.004, 0.0001))
    #train and evaluate
    ...
    np.save(wdir+'chaskey_h'+str(num_rounds)+'r_pairs' +
            str(group_size)+'.npy', h.history['val_acc'])
    np.save(wdir+'chaskey_h'+str(num_rounds)+'r_pairs' +
            str(group_size)+'.npy', h.history['val_loss'])
    dump(h.history, open(wdir+'chaskey_hist'+str(num_rounds)+'r_pairs'+str(group_size)+'.p', 'wb'))
