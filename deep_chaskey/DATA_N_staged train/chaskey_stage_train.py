...
def first_stage(pairs):
    with mp.Pool(process_number) as pool:
        accept_XY = pool.starmap(chk.create_train_test_dataset, [(int(10**7/process_number), int(
            10**6/process_number), num_rounds, 0, 0, pairs, (0x8400, 0x0400, 0, 0),) for i in range(process_number)])
    X = accept_XY[0][0]
    Y = accept_XY[0][1]
...
def second_stage(pairs):
    ...
    with mp.Pool(process_number) as pool:
        accept_XY = pool.starmap(chk.create_train_test_dataset, [(int(10**7/process_number), int(
            10**6/process_number), num_rounds+1, 0, 0, pairs, (0x8400, 0x0400, 0, 0),) for i in range(process_number)])
    X = accept_XY[0][0]
    Y = accept_XY[0][1]
    X_eval = accept_XY[0][2]
    Y_eval = accept_XY[0][3]
